    ...
    algorithms: [
      {
        ...
        name: "复合加权模型",
        weight: "最终输出",
        description: "将以下四个维度按权重融合，归一化后输出最终概率分布。比分由泊松预期进球+ELO净胜球差联合推算，并确保胜负方向一致。",
        factors: ["ELO 35%", "泊松分布 30%", "近期形态 25%", "趋势修正 10%"]
      },
      {
        id: "elo",
        name: "1. ELO评级模型",
        weight: "35%",
        description: "基于FIFA世界排名换算的基础ELO分（阿根廷1920→海地1575），叠加主场优势+50分修正。ELO差值通过标准公式 P(home)=1/(1+10^(-Δ/400)) 转化为胜率，平局概率随实力接近度增加（ELO差<100时平局概率最高32%）。每100分ELO差≈预期净胜球0.5球。",
        factors: ["FIFA排名换算ELO", "主场优势+50", "ΔELO→胜率公式", "平局概率自适应"]
      },
      {
        id: "poisson",
        name: "2. 泊松分布模型",
        weight: "30%",
        description: "基于近10场国家队数据的攻防系数（att/def），计算预期进球：xG_home = att_h × (def_a/1.2) × 主场系数1.15，xG_away = att_a × (def_h/1.2)。对0~7球区间做全枚举泊松概率 P(k)=λ^k·e^(-λ)/k!，乘积得比分概率矩阵，取概率最大的比分作为预测。",
        factors: ["进攻力×对手防守", "主场优势×1.15", "泊松枚举0~7球", "概率矩阵取max"]
      },
      {
        id: "form",
        name: "3. 近期形态分析",
        weight: "25%",
        description: "取本届世界杯已赛数据：积分效率(50%) + 平均进球(30%) + 净胜球(20%) 加权得形态分。形态分归一化后算胜率占比。未赛球队默认0.45基准分。对手强度暂未纳入(待小组赛结束后引入SoS修正)。",
        factors: ["小组赛积分效率", "场均进球", "净胜球", "0.45冷启动基准"]
      },
      {
        id: "trend",
        name: "4. 趋势修正因子",
        weight: "10%",
        description: "定性修正：出线/淘汰动力差（已出线球队可能轮换→动力下调）、连胜/连败惯性、同大洲内战心理因素、阵容深度与伤病叠加。当前权重较低，作为模型对纯数据信号的微调项。",
        factors: ["出线动力差", "连胜/连败惯性", "阵容伤病叠加", "大赛经验"]
      },
      {
        id: "liuyao",
        name: "🧿 六爻占卜（术数辅助）",
        weight: "独立展示",
        description: "每场比赛独立起卦（队伍编码+比赛时间→三数起卦），纳甲专业排盘后取世爻(主队)/应爻(客队)旺衰值，结合动爻生克、六亲映射（妻财=进攻/官鬼=防守/子孙=创造力）、六兽警示（白虎→红牌/玄武→争议判罚）做综合推断。比分由ELO预期进球+火力系数+动爻扰动经泊松采样得出。把握度由世应差距+动爻强度+概率集中度三维加权。",
        factors: ["三数独立起卦", "世应旺衰对比", "动爻生克+六亲映射", "六兽风险警示", "ELO泊松联合比分"]
      }
    ],
    confidenceNote: "置信度≥70%为高把握预测，45-69%为中把握，<45%为低把握（可能出现冷门）。复合模型与六爻占卜独立计算，可交叉参考。"
  };
}
